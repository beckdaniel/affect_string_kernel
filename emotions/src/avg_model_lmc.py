...
#if args.bias:
#    mode += '_bias'
main_out_dir = os.path.join(args.output_dir, 'avg', 'lmc', args.model + '_' + mode, args.label_preproc)
...
for i_train, i_test in folds:
    ...
    if args.model == 'ridge':
        ...
        lmck = GPy.util.multioutput.LCM(input_dim=X.shape[1], num_outputs=6,
                                        kernels_list=[k, GPy.kern.Bias(X.shape[1])],
                                        W_rank=args.rank)
